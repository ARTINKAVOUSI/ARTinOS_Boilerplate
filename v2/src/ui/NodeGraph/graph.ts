/**
 * The node graph model: schema, validation and evaluation.
 *
 * Nothing here touches React, the DOM or any host application. A graph reads
 * and writes the outside world only through the `GraphIO` callbacks the host
 * passes to `evaluate`, so the same graph runs against a studio, a test or a
 * plain page.
 */

export type GraphValueType = 'number' | 'boolean'
...
export interface GraphField {
  id: string
  label: string
  kind: 'number' | 'select' | 'text' | 'signal' | 'parameter'
...
export type GraphCategory = 'Source' | 'Math' | 'Shape' | 'Logic' | 'Output'
...
/** How a graph reaches the world outside itself. Every call is the host's. */
export interface GraphIO {
  /** Live value of a named signal, e.g. `audio.bass`. */
  readSignal: (id: string) => number
  /** Current value of a named parameter, e.g. `effect.bloom:strength`. */
  readParameter: (id: string) => number
  writeSignal: (id: string, value: number) => void
  writeParameter: (id: string, value: number) => void
}

export interface GraphContext extends GraphIO {
...
const port = (id: string, extra: Partial<GraphPort> = {}): GraphPort => ({ id, label: title(id), type: 'number', ...extra })
const out: readonly GraphPort[] = [port('value')]
...
const unary = (type: string, label: string, description: string): GraphNodeSchema => ({ type, label, category: 'Math', description, inputs: [port('value')], outputs: out, fields: [] })

export const NODE_SCHEMAS: readonly GraphNodeSchema[] = [
  // Sources
  { type: 'constant', label: 'Constant', category: 'Source', description: 'A fixed number.', inputs: [], outputs: out, fields: [num('value', 1)] },
  { type: 'signal', label: 'Signal', category: 'Source', description: 'Reads a live signal such as audio.bass or pointer.x.', inputs: [], outputs: out, fields: [{ id: 'id', label: 'Signal', kind: 'signal', default: 'audio.level' }] },
  { type: 'parameter', label: 'Parameter', category: 'Source', description: 'Reads the current value of a control.', inputs: [], outputs: out, fields: [{ id: 'id', label: 'Parameter', kind: 'parameter', default: '' }] },
  { type: 'time', label: 'Time', category: 'Source', description: 'Seconds since the graph started, scaled and offset.', inputs: [], outputs: out, fields: [num('speed', 1), num('offset', 0)] },

  // Math
  { type: 'add', label: 'Add', category: 'Math', description: 'Sums every connected input.', inputs: [port('values', { multiple: true })], outputs: out, fields: [] },
  { type: 'multiply', label: 'Multiply', category: 'Math', description: 'Multiplies every connected input.', inputs: [port('values', { multiple: true })], outputs: out, fields: [] },
  { type: 'min', label: 'Min', category: 'Math', description: 'Smallest connected input.', inputs: [port('values', { multiple: true })], outputs: out, fields: [] },
  { type: 'max', label: 'Max', category: 'Math', description: 'Largest connected input.', inputs: [port('values', { multiple: true })], outputs: out, fields: [] },
  { type: 'subtract', label: 'Subtract', category: 'Math', description: 'A minus B.', inputs: [port('a'), port('b')], outputs: out, fields: [num('b', 0)] },
  { type: 'divide', label: 'Divide', category: 'Math', description: 'A divided by B. Dividing by zero yields A.', inputs: [port('a'), port('b')], outputs: out, fields: [num('b', 1)] },
  { type: 'modulo', label: 'Modulo', category: 'Math', description: 'Remainder of A divided by B.', inputs: [port('a'), port('b')], outputs: out, fields: [num('b', 1)] },
  { type: 'pow', label: 'Power', category: 'Math', description: 'A raised to B.', inputs: [port('a'), port('b')], outputs: out, fields: [num('power', 2)] },
  unary('abs', 'Absolute', 'Distance from zero.'),
  ...
  unary('sin', 'Sine', 'Sine of the input in radians.'),
  unary('cos', 'Cosine', 'Cosine of the input in radians.'),

  // Shaping
  { type: 'remap', label: 'Remap', category: 'Shape', description: 'Rescales an input range onto an output range.', inputs: [port('value')], outputs: out, fields: [num('inMin', 0), num('inMax', 1), num('outMin', 0), num('outMax', 1)] },
  { type: 'clamp', label: 'Clamp', category: 'Shape', description: 'Holds the input inside a range.', inputs: [port('value')], outputs: out, fields: [num('min', 0), num('max', 1)] },
  { type: 'smoothstep', label: 'Smoothstep', category: 'Shape', description: 'Eased 0–1 ramp between two edges.', inputs: [port('value')], outputs: out, fields: [num('min', 0), num('max', 1)] },
  { type: 'mix', label: 'Mix', category: 'Shape', description: 'Blends A and B by T.', inputs: [port('a'), port('b'), port('t')], outputs: out, fields: [num('t', 0.5, { min: 0, max: 1 })] },
  { type: 'noise', label: 'Noise', category: 'Shape', description: 'Deterministic −1..1 value hashed from the input, or from time when unwired.', inputs: [port('value')], outputs: out, fields: [num('frequency', 1), num('seed', 0)] },
  {
    type: 'oscillator',
    label: 'Oscillator',
    category: 'Shape',
  ...
  { type: 'smooth', label: 'Smooth', category: 'Shape', description: 'Frame-rate independent one-pole smoothing. 0 passes through, 1 holds still.', inputs: [port('value')], outputs: out, fields: [num('amount', 0.85, { min: 0, max: 0.999 })] },
  ...
  // Outputs
  { type: 'write-parameter', label: 'Write Parameter', category: 'Output', description: 'Drives a control while the graph runs. The authored value comes back when it stops.', inputs: [port('value', { required: true })], outputs: out, fields: [{ id: 'id', label: 'Parameter', kind: 'parameter', default: '' }] },
  { type: 'write-signal', label: 'Write Signal', category: 'Output', description: 'Publishes the input as a named signal other graphs and features can read.', inputs: [port('value', { required: true })], outputs: out, fields: [{ id: 'id', label: 'Signal', kind: 'text', default: 'graph.out' }] },
]

export const GRAPH_CATEGORIES: readonly GraphCategory[] = ['Source', 'Math', 'Shape', 'Logic', 'Output']
...
export const defaultData = (type: string): Record<string, string | number> => Object.fromEntries((schemaFor(type)?.fields ?? []).map(field => [field.id, field.default]))

let sequence = 0
export function createNode(type: string, x = 0, y = 0, data: Record<string, string | number> = {}): GraphNode {
...
/** Edges feeding one input port. */
export const edgesInto = (graph: GraphDefinition, nodeId: string, inputId: string) => graph.edges.filter(edge => edge.to === nodeId && (edge.input ?? 'value') === inputId)
...
export function validateGraph(graph: GraphDefinition): GraphDiagnostic[] {
  ...
  for (const node of graph.nodes) if (!schemaFor(node.type)) diagnostics.push({ severity: 'error', message: `Unknown node type ${node.type}`, nodeId: node.id })
  for (const edge of graph.edges) {
    const from = nodes.get(edge.from)
    const to = nodes.get(edge.to)
    if (!from || !to) diagnostics.push({ severity: 'error', message: 'This wire points at a node that is gone', edgeId: edge.id })
  }
  for (const node of graph.nodes) {
    for (const input of inputsOf(node)) {
      const wired = edgesInto(graph, node.id, input.id).length
      if (input.required && !wired) diagnostics.push({ severity: 'error', message: `${schemaFor(node.type)?.label ?? node.type}: ${input.label ?? input.id} must be connected`, nodeId: node.id })
    }
    if ((node.type === 'write-parameter' || node.type === 'parameter') && !String(fieldValue(node, 'id') ?? '')) diagnostics.push({ severity: 'warning', message: 'No parameter chosen', nodeId: node.id })
    if (node.type === 'signal' && !String(fieldValue(node, 'id') ?? '')) diagnostics.push({ severity: 'warning', message: 'No signal chosen', nodeId: node.id })
  }
...
/** Which parameters this graph writes — the host restores them when it stops. */
export const drivenParameters = (graph: GraphDefinition): string[] =>
  graph.nodes.filter(node => node.type === 'write-parameter').map(node => String(fieldValue(node, 'id') ?? '')).filter(Boolean)
 ...
 * Output nodes write through the context; everything else is pure.
 */
export function evaluate(graph: GraphDefinition, context: GraphContext): Map<string, number> {
  const values = new Map<string, number>()
  const wired = (node: GraphNode, portId: string) => edgesInto(graph, node.id, portId).map(edge => values.get(edge.from)).filter((value): value is number => value !== undefined)
