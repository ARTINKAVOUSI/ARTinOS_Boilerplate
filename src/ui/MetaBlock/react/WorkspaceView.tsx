...
/** Releasing a pointer the element never captured throws; a synthetic or cancelled gesture can get here. */
/** Capturing a pointer that is no longer active throws (synthetic events, a gesture already ended). */
const captureSafely=(event:PointerEvent<HTMLElement>)=>{try{event.currentTarget.setPointerCapture?.(event.pointerId)}catch{/* keep the gesture uncaptured */}};
const releaseCapture=(event:PointerEvent<HTMLElement>)=>{const target=event.currentTarget;if(target.hasPointerCapture?.(event.pointerId))target.releasePointerCapture(event.pointerId)};
...
export function MetaBlockWorkspaceView({
  ...
  const beginGroupDrag=(event:PointerEvent<HTMLElement>,groupId:string)=>{
    ...
    captureSafely(event);workspace.focusGroup(groupId);setContextMenu(null);event.preventDefault();
  };
  ...
  const blockProps=(groupId:string,blockId:string)=>({
    ...
    onPointerUp:(event:PointerEvent<HTMLElement>)=>{const d=blockDragRef.current;if(!d||d.pointerId!==event.pointerId)return;blockDragRef.current=null;releaseCapture(event);const wasStarted=d.started,directAttach=d.candidate?.kind==='merge'&&(d.candidate.relation==='home'||d.candidate.relation==='dock')&&d.candidate.strength>=.56?d.candidate:null,c=d.commitCandidate&&d.commitCandidate.strength>=commitStrength?d.commitCandidate:directAttach,ghost=d.ghost;clearInteraction();if(!wasStarted){workspace.activateBlock(blockId);return}
  ...
  const endResize=(event:PointerEvent<HTMLElement>)=>{if(resizeRef.current?.pointerId!==event.pointerId)return;resizeRef.current=null;setResizeGuide(null);releaseCapture(event)};
