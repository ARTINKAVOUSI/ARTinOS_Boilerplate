...
import { resolvePostFXCapability, usePostFXController, TRANSMISSION_BACKDROP_RESOURCE, type PostFXEffect, type PostFXRuntimeState } from './postfx'

const nodes = TSL as Record<string, any>
const { float, mrt, normalView, output, pass, posterize, vec2, vec4, velocity, grayscale, hue, saturation, packNormalToRGB } = nodes
...
export function RenderPipelineSystem(){
  ...
  const needsNormal=controller.enabled&&effects.some(e=>MRT_NORMAL.has(e.type)),needsVelocity=controller.enabled&&effects.some(e=>MRT_VELOCITY.has(e.type)),needsPackedNormal=controller.enabled&&effects.some(e=>e.type==='recurrentDenoise')
  const state=useMemo(()=>{
    ...
    scenePass.setMRT(needsNormal||needsVelocity?mrt({output,...(needsNormal?{normal:normalView}:null),...(needsPackedNormal?{normalPacked:packNormalToRGB(normalView)}:null),...(needsVelocity?{velocity}:null)}):null)
  ...
  },[renderer,scene,camera,needsNormal,needsVelocity,needsPackedNormal])
